<template>
    <div dir="rtl" class="overflow-x-auto bg-gray-50 border-2 border-gray-200 rounded-2xl py-8 px-2">
      <table class="table-auto w-full text-center border-collapse text-sm border-t border-gray-300">
        ...
        <tbody>
          <tr v-for="bid in currentBids" :key="bid.id" class="border-b">
            <td class="px-8 py-2">
            ...
            <td class="px-8 py-2" :class="{'text-green-500': bid.status === 'الأعلى', 'text-gray-500': bid.status !== 'الأعلى'}">
              {{ bid.status }}
            </td>
          </tr>
        </tbody>
  ...
  export default {
    data() {
      return {
        selectAll: false,
        currentBids: [
          { id: 1, bidder: "أحمد محمد", amount: 5000, time: "12:30:45", status: "الأعلى", selected: false },
          { id: 2, bidder: "سارة علي", amount: 4800, time: "12:29:30", status: "سابق", selected: false },
          { id: 3, bidder: "خالد عمر", amount: 4600, time: "12:28:15", status: "سابق", selected: false },
        ]
      };
    },
    methods: {
      toggleAll() {
        this.currentBids.forEach(bid => bid.selected = this.selectAll);
      },
      updateSelectAll() {
        this.selectAll = this.currentBids.every(bid => bid.selected);
      }
    }
  };
  </script>